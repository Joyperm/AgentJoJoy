...
Every rule is numbered (e.g. `SPEC-1.2`) so other docs can reference
it precisely. The numbers are **stable identifiers**; the groups below convey
priority and meaning.

## Table of SPEC Groups

| Group (priority order) | Purpose | SPECs |
| :--- | :--- | :--- |
| **A. Safety & Authority** | Non-negotiable gates: what needs approval, what's reserved for the user, how to decide. | SPEC-1 (Permission Gates), SPEC-2 (Verification Hierarchy) |
| **B. Foundations & Lifecycle** | Shared vocabulary and the normal flow of one task. | SPEC-3 (Vocabulary), SPEC-4 (Task Lifecycle) |
| **C. Sync & Parallel Work** | Keeping branches current and running multiple tasks at once. | SPEC-5 (Upstream Sync), SPEC-6 (Parallel Worktrees) |
| **D. Recovery, Cleanup & Docs** | When things break, finishing up, and keeping docs honest. | SPEC-7 (Failure Recovery), SPEC-8 (Cleanup), SPEC-9 (Documentation Discipline) |

---

## SPEC-1. AI Permission Gates
*Group A — Safety & Authority.*
...
### SPEC-1.2 — Safe without approval
Reads and read-only inspection run freely: file reads; `git status`/`log`/`diff`/
`branch --list`/`worktree list`/`ls-files`/`check-ignore`/`remote -v`; `git fetch`
(incl. `--prune`); documented local tests/type-checks/builds; process/network
inspection; authorized read-only non-prod DB queries. Full list:
`ai-workflow-rules.md` → Pillar I "Safe to do without asking".
...
### SPEC-1.5 — Strategic choice gate
Execution authority (allowlists/hooks that let a command run without prompting)
does **not** grant strategic-choice authority: *which* command to run among valid
options (rebase vs merge, commit/stash/discard, conflict side, branch base, cleanup
timing, force vs not, which file to delete, ship-vs-block) is the user's to name
every time — list options, explain trade-offs, wait for an explicit named choice,
then propose under SPEC-1.1. Canonical detail: `ai-workflow-rules.md` → Pillar I
"Strategic choices require explicit selection (SPEC-1.5)".

### SPEC-1.6 — Force push safety
...
### SPEC-1.7 — Block Protection (AI-NO-OVERWRITE)
Text wrapped in `<!-- AGENTJOJOY:AI-NO-OVERWRITE BEGIN -->` … `END` is protected
from **autonomous** AI modification — never edit, delete, or move it (or the tags)
unless the user explicitly requests that specific change; propose and wait first.
User-directed edits are allowed. Canonical detail: `ai-workflow-rules.md` →
...
3. Does the action require approval per SPEC-1.1? → ask
4. Is the action a documented step in SPEC-4? → propose it as such
5. Otherwise → proceed (SPEC-1.2)
...
## SPEC-3. Vocabulary
*Group B — Foundations & Lifecycle.*

The following terms have specific meanings in this document:

- **SPEC-3.1 — Upstream repo**: the canonical git remote (e.g. a
...
- **SPEC-3.2 — Main checkout**: the local clone of the upstream repo
...
- **SPEC-3.3 — Worktree**: a separate working directory that shares
...
- **SPEC-3.6 — Task branch**: the branch dedicated to a single task,
  ...
  `workspace-model.md`.
- **SPEC-3.7 — AI**: the Claude (or other) agent driving the
  workflow under user direction.
- **SPEC-3.8 — User**: the human directing the AI.

---

## SPEC-4. Lifecycle of One Task
*Group B — Foundations & Lifecycle.*
...
```
4.1 Intake → 4.2 Pre-flight → 4.3 Worktree creation → 4.4 Setup →
4.5 Implementation → 4.6 Verification → 4.7 Push → 4.8 PR → 4.9 Review
loop → 4.10 Merge → 4.11 Cleanup
```
...
- **Target worktree must be part of the restatement when multiple
  worktrees are active** (parallel mode per SPEC-6). If the task
  ...
  is needed. This is a SPEC-1.5 strategic choice and may not be
...
- The AI proposes:
  ...
  - The task branch name (per SPEC-3.6 naming)
...
### SPEC-4.9 Review loop
- While the PR is awaiting review, the worktree stays alive.
- If the reviewer requests changes:
  - The AI edits in the same worktree
  - The AI follows SPEC-4.6 (verify) and SPEC-4.7 (push) again
- The AI may start another task in parallel — see SPEC-6.

### SPEC-4.10 Merge
- After merge, the AI runs `git fetch origin --prune` (safe; no
  approval needed under SPEC-1.2).
...
## SPEC-5. Upstream Sync Protocol
*Group C — Sync & Parallel Work.*
...
### SPEC-5.1 Detection
- The AI runs `git fetch origin` (safe under SPEC-1.2).
...
### SPEC-5.2 Pre-sync state assessment
...
### SPEC-5.3 Strategy choice
This is a SPEC-1.5 strategic choice — the AI must ask explicitly
...
does not replace the user's explicit selection under SPEC-1.5.

The user must select explicitly; a generic "go" is insufficient (see
SPEC-1.5).

### SPEC-5.4 Handling uncommitted changes pre-sync
...
- After all conflicts resolved, the AI runs `git rebase --continue` or
  completes the merge (these are approval-gated under SPEC-1.1)

### SPEC-5.6 Verification post-sync
After the sync completes:

- The AI runs the project's verification commands (SPEC-4.6)
- The AI reports any failures to the user
- If verification fails, the AI does not push the synced branch

---

## SPEC-6. Parallel Worktrees
*Group C — Sync & Parallel Work.*

### SPEC-6.1 Multiple worktrees may coexist
...
### SPEC-6.2 No branch switching across worktrees
...
### SPEC-6.3 Cross-worktree awareness
...
## SPEC-7. Failure Recovery
*Group D — Recovery, Cleanup & Docs.*

### SPEC-7.1 Pre-commit hook failure
...
### SPEC-7.3 Push rejected (non-fast-forward)
...
- Enter the SPEC-5 upstream sync protocol
...
- The AI must **NOT** propose `git push --force` or
  ...
  even after sync. See SPEC-1.6.

### SPEC-7.4 Lost work prevention
...
## SPEC-8. Cleanup Protocol
*Group D — Recovery, Cleanup & Docs.*

### SPEC-8.1 When to cleanup
...
The work tracker must stay a short, high-level snapshot of current state — not a
blow-by-blow journal. Unbounded appending until the file is unreadable is a known
failure mode.

Rules:
- **Git log is the audit trail, not the tracker.** Detailed step history lives in
  commit messages (especially milestone commits under SPEC-1.8). To recover
  fine-grained history, run `git log` rather than transcribing each step into the
  tracker.
- **Summarize on completion.** When a task finishes, collapse its in-flight notes
  into a brief summary (what shipped + outcome), **replacing** the working notes
  rather than appending to them.
- **Update on state changes, not every action.** Update for meaningful transitions
  (branch/PR created, merged, blocker found, task done) — not after each small edit.
...
### SPEC-9.4 — Documentation Conciseness

Workspace rule/context docs must stay tight enough that agents actually read them —
verbose docs get skimmed, and skimmed rules get skipped. When writing or editing one:

- **Lead with the rule.** First sentence states the requirement; rationale follows.
- **Front-load** the highest-stakes rules at the file head (e.g. a Quick Reference),
  for agents that read only the top.
- **Cut filler.** One idea per bullet; drop hedging and repeated rationale; prefer
  tables/short bullets over prose.
- **Concise ≠ lossy.** Preserve every nuance, exception, and cross-reference; never
  break anchors/links. Point to the canonical file instead of duplicating its detail.

Principle: "light but enforced" beats "heavy but skipped."
